...
def grid_as_svg(grid, width, height):
    ...
    for irow in range(nrows):
        ...
        for icol in range(ncols):
            ...
            if letter != ' ':
                s.append(
                    '<text x="{}" y="{}" text-anchor="middle">{}</text>'.format(x, y, letter))
            x += letter_width
        y += letter_height
...
wordlist = sorted(get_wordlist(wordlist_filename),
...
if max(len(word) for word in wordlist) > max_word_len:
    raise ValueError(
        'Word list contains a word with too many letters.''The maximum is {}'.format(max(nrows, ncols)))
...
grid, solution = make_wordsearch(
    nrows, ncols, wordlist, allow_backwards_words, mask)
